...
Table::Table(CameraInterface& cam, ColorTracker& colTrack, ContourTracker& conTrack, int sampleFreq) : 
	...
	sampleFreq(sampleFreq) {
		...
		stopSample = FALSE;
		bounceListI = 0;
		top_left = cv::Point2f(28,8);
		bottom_right = cv::Point2f(290,138);
		...
		Hue_Lower_Value = HUE_LOWER;//initial hue value(lower)//
		Hue_Upper_Value = HUE_UPPER;//initial hue value(upper)//
		Saturation_Lower_Value = SAT_LOWER;//initial saturation(lower)//
		Saturation_Upper_Value = SAT_UPPER;//initial saturation(upper)//
		Value_Lower = VAL_LOWER;//initial value (lower)//
		Value_Upper = VAL_UPPER;//initial saturation(upper)//
